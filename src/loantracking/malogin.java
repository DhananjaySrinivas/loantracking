...
public class malogin extends HttpServlet{
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException,ServletException{
		...
		try {
            ...
            String pas="select * from managerlogin where name='"+s+"' and password='"+s1+"'";
			...
			{	
			name=se.getString(1);
			 password=se.getString(2);
		}
			 if (name.equals(s) && password.equals(s1)) 
			 {	
				 System.out.println("login successfull");
				 response.getWriter().append("Served at: ").append(request.getContextPath());
				 RequestDispatcher rd = request.getRequestDispatcher("managerdashboard.html");
